...
1. iOS Safari: Get Started Email filed edition issue (https://github.com/shipttask/assesmentqa/blob/master/iOS_safafi_email_field_issue.MP4)
...
2. iOS Native app: Select Store navigation issue (https://github.com/shipttask/assesmentqa/blob/master/iOS_app_navigation_issue.MP4)
...
# 4.
Customers table relates to Orders table like '1 to many'. Shoppers table relates to Orders table like '1 to many'. (https://github.com/shipttask/assesmentqa/blob/master/tables_relations.pdf)
SQL query to select ID and Store tied to a specific Customer and Shopper in https://github.com/shipttask/assesmentqa/blob/master/IDandSTORE.sql script. Visualisation in https://github.com/shipttask/assesmentqa/blob/master/sql_select.PNG.
...
2. Istall Sahi Test Framework, after first run install the license  (https://github.com/shipttask/assesmentqa/blob/master/license.data).
...
5. After the test scipt finishes, navigate to Sahi Logs to verify results (https://github.com/shipttask/assesmentqa/blob/master/Reports_archive.zip).

# 6.
I selected Sahi Test framework because it is a straightforward testing tool to maintain, develop, run and debug test scripts. Sahi is written on Java and can be run on all platforms. Sahi supports recording steps and provides plenty of predefined selectors, so it is easy to write test scripts on a fly. Sahi uses modified JavaScript and supports REST API. Sahi generates usefull reports, i.e., in case of found issue, Sahi conveniently makes screenshots. No need to define wait statements for AJAX, it's done in the framework itself. All scripts can be run in parallel which reduces time spent on running.
